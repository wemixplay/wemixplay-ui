    ...
    if (!list || loading) {
      ...
      if (!skeletonElement) {
        ...
        return Array.from({ length: skeletonCnt }).map((_, index) => {
          return cloneElement(skeletonElement, { ...skeletonElement.props, key: index });
        });
      }
      ...
      return typeof noDataMsg === 'object' ? (
        ...
        <NoDataText className={cx('no-data')} nullText={noDataMsg} />
      );
    } else {
      /**
       ...
       * </ListMap>
       */
      return list.map((item, index) => children({ item: item as GetArrayItemType<LIST>, index }));
    }
  }, [list, skeletonElement, loadingElement, skeletonCnt, noDataMsg, loading, children]);

  return <ErrorBoundary fallback={!errorFallback || loadingElement}>{renderList}</ErrorBoundary>;
};
