...
import Person, { PersonProps } from '../avatars/Person';
 ...
 */
const AvatarButton = ({ className = '', size = 'medium', onClick, ...personProps }: Props) => {
  //logic

  return (
    <button className={cx(className, 'avatar-button', size)} type="button" onClick={onClick}>
      <Person {...personProps} className={cx('avatar-button-person')} size={size} />
    </button>
  );
};
